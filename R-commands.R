...
setwd("/state/partition1/freddyhorna/")
...
subset.deg.df <- subset(deg.df, (((logFC >= 1) | (logFC <= -1)  ) & (FDR <=0.05))  )
...
my_palette <- colorRampPalette(c("white", "gray", "blue"))(n = 299)
...
png("./output/abundance/DEG/heatmap_DEGS_SAMPLEA-SAMPLEB.png",    # cria arquivo do tipo PNG for the heat map        
    ...
    pointsize = 5)        # tamanho da fonte

heatmap.2(log2(expression_data), # a matriz com os valores de expressão
          #main = "Correlation", # Título do HeatMap
          main = "Distância Euclideana",
          density.info="none",  # desabilita o gráfico de densidade dentro da legenda
          trace="none",         # desabilita as linhas dentro do HeatMap
          margins =c(24,36),     # definiação das margens no entorno do gráfico
          ...
          hclustfun = function(x) hclust(x, method="average") # método de ligação pelo centróide
)
